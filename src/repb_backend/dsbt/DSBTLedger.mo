...
*/
    ...
    public query func icrc7_balance_of(account : Account) : async Tokens {
        balance(account, log);
    };

    public query func icrc7_total_supply() : async Tokens {
        totalSupply(log);
    };

    public query func icrc7_minting_account() : async ?Account {
    ...
    public query func icrc7_decimals() : async Nat8 {
        init.decimals;
    };

    public query func icrc7_fee() : async Nat {
        init.transfer_fee;
    };

    public query func icrc7_metadata() : async [(Text, Value)] {
        [
            ("icrc7:name", #Text(init.token_name)),
            ("icrc7:symbol", #Text(init.token_symbol)),
            ("icrc7:decimals", #Nat(Nat8.toNat(init.decimals))),
            ("icrc7:fee", #Nat(init.transfer_fee)),
        ];
    };

    public query func icrc7_supported_standards() : async [{
        name : Text;
        url : Text;
    }] {
        [
